...
public class PlayerDiameter : MonoBehaviour
{
    //拡大していく速さ
    [SerializeField]
    ...
    //Playerに子要素が無いときの直径(一番最初の直径)
    private float firstDimeter;
    //Playerと子要素を合わせた直径
    private float playerDimeter;
    ...
    void Start()
    {
        ...
        firstDimeter = sphereCollider.radius * 2;
        playerDimeter = firstDimeter;
        currentDimeter = firstDimeter;
    }
    ...
    void Update()
    {
        ...
        //現在の直径と更新している直径を比べる
        if (currentDimeter < playerDimeter)
        {
            //大きい方に合わせる
            currentDimeter = playerDimeter;
        }
        ...
        {
            //当たったら最初の状態に戻す
            sphereCollider.radius = firstDimeter / 2;
        }
    ...
    void OnTriggerStay(Collider collider)
    {
        if (collider.transform.gameObject == playerScript.lastChildObj)
        {
            ...
            //Playerの子要素を含めた直径 = 最初の直径 + (大きくなったコライダーの半径 - 最初の半径)
            playerDimeter = firstDimeter + (sphereCollider.radius - (firstDimeter / 2));
